 ...
 * @constructor
 */
function GetLeft(elm){               //  vimame otmestvaneto na canvas-a
 ...
 * @constructor
 */
Array.prototype.remove = function(arg, all){   // iztrivame purviq element ot arr i go iztrivame
 ...
 * @constructor
 */
Array.prototype.clear = function(){  //iztriva vsi4ko ot arr
    this.length = 0;
};
 ...
 * @constructor
 */
Array.prototype.contains = function(arg){  // proverqva dali tozi element su6testvuva
    for(var i = 0; i < this.length; i++)
        if(this[i] == arg)
            return true;
    return false;
 ...
 * @constructor
 */
Array.prototype.occurs = function(arg){  // broi kolko puti imame el v masiva
    var counter = 0;
...
var Vector2 = (function() {// Obekt, koito pazi x i y, ima i predi6nite x,y koito sme zadali
     ...
     * @param y
     */
    Vector2.prototype.set = function(x, y) {    // sluji za setvane na stoinosti na vektora
        if(!x && !y) {
     ...
     * @param vec2
     */
    Vector2.prototype.move = function(vec2) {  // podavame vektor i uvale4avame stoinostite na na6iq vektor s tezi na drugiq
        this.x += vec2.x;
        this.y += vec2.y;
    };
     ...
     * @returns {number}
     */
    Vector2.prototype.distance = function(vec2) {   // iz4islqva razstoqnieto ot edna do druga to4ka
        if(vec2)
     ...
     * @returns {boolean}
     */
    Vector2.prototype.hasChanged = function() {  // proverqva dali ima promqna vuv vektora
     ...
     * @returns {Vector2}
     */
    Vector2.prototype.difference = function(vec2, invert) { //  vadi ot edna to4ka druga
        var inv = 1;
...
var Color = (function(){   // podavame mu stoinosti i ni pravi cvqt
...
var Rectangle = (function() {
     ...
     * @returns {boolean}
     */
    Rectangle.prototype.contains = function(x, y) {   //  proverqva dali v ediniq kvadrat ima drug kvadrat
     ...
     * @returns {boolean}
     */
    Rectangle.prototype.intersects = function(shape) {  // ot krug se suzdava pravougulnik i proverqva dali se zasi4at s drug pravougulnik
        var offset = 0;
     ...
     * @param ctx
     */
    Rectangle.prototype.draw = function(ctx) {   // risuva prevougulnika
...
var Animation = (function() {  // podavame URL i imame animaciq,
    function Animation(width, height, row, column, limit, imgSrc, fps, columns, rows) {
        if(!fps || fps >=33) {  // 'row' i 'column' e startovata poziciq ot koqto po4va da
            this.fps = 1;       // zapo4ne spraita, 'limit' ni kazva kolko na broi kartinki da pokazva
        } else {                // 'fps' e skorosta na animaciqta za sekunda
            this.fps = 33 / fps;// 'colums' i 'rows' sa kolonite i redovete prez koito iskame da ciklim
        }
     ...
     * @param num
     */
    Animation.prototype.setColumn = function(num) { // smenqme poziciqta ot koqto isme da rejem
        this.column = num;
     ...
     * @constructor
     */
    Animation.prototype.update = function() { // update e da se cikli
        this.cropPostion.x = this.width * this.column;
        this.cropPostion.y = this.height * this.row;
     ...
     * @param ctx
     */
    Animation.prototype.draw = function(ctx) { // izrsuvaneto na kartinkite
